<template>
  <div class="min-h-screen relative flex flex-col overflow-hidden signIn-page">
    ...
    <div class="flex-1 flex flex-col items-center justify-center relative z-10 p-8">
      <!-- Sign in card -->
      <div class="bg-white  shadow-lg w-full max-w-[440px] min-w-[320px] min-h-[338px] -mt-6 mb-5 p-11">
        <!-- Logo -->
        <div >
        ...
        <Transition :name="transitionName" mode="out-in">
          ...
          <div v-if="!isPasswordStep" key="email-step" class="w-full">
            <h1 class="text-2xl font-semibold text-gray-900 mb-3 mt-4 h-7">Sign in</h1>
          ...
          <div v-else key="password-step" class="w-full">
            <div class="">
              ...
              >
                <img src="/assets/arrow_left.svg" alt="Back" class=" w-6 h-6 mt-0.5" />
                <span class=" text-sm">{{ email }}</span>
              </button>
            </div>
            <h1 class="text-2xl font-semibold text-gray-900 mt-4 mb-3">Enter password</h1>
      ...
      <Transition :name="transitionName">
        <div v-if="!isPasswordStep" class=" w-full max-w-[440px]">
          <button 
            class="w-full px-11 py-2 bg-white shadow-xl flex items-center space-x-3  opacity-50 "
            disabled
          >
            <img src="/assets/key.svg" alt="Sign-in options" class=" w-8 h-8 text-gray-600" />
    ...
    <!-- Footer -->
    <footer class="absolute bottom-0 right-0 flex items-center space-x-6 pt-3 pr-6 z-10">
      <a :href="defaultLink" class="text-xs text-gray-700 hover:underline">Terms of use</a>
      <a :href="defaultLink" class="text-xs text-gray-700 hover:underline">Privacy & cookies</a>
      <a 
        :href="defaultLink"

      >
        <svg class="w-5 h-5 mt-1 text-gray-600" fill="currentColor" viewBox="0 0 24 24">
